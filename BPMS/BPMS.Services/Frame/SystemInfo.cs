...
namespace BPMS.Services
{
    public partial class Service
    {
        public string SystemGetList(string xmlCredentials)
        {
            ...
            if (CheckPurview(objCredentials, EModules.SystemApply, EFunctions.SystemMng, EActions.Vie) != 1)
        ...
        {
            ClientCredentials objCredentials = xmlCredentials.ToModel<ClientCredentials>();
            if (CheckPurview(objCredentials, EModules.SystemApply, EFunctions.SystemMng, EActions.Add) != 1)
                throw new Exception(String.Format("Service Method:{0} Access Error", base.GetActionName()));
        ...
        {
            ClientCredentials objCredentials = xmlCredentials.ToModel<ClientCredentials>();
            if (CheckPurview(objCredentials, EModules.SystemApply, EFunctions.SystemMng, EActions.Upd) != 1)
                throw new Exception(String.Format("Service Method:{0} Access Error", base.GetActionName()));
            var model = xmlModel.ToModel<SystemInfo>();
            model.ModifyDate = DateTime.Now;
            model.ModifyUserId = objCredentials.UserId;
        ...
        {
            ClientCredentials objCredentials = xmlCredentials.ToModel<ClientCredentials>();
            if (CheckPurview(objCredentials, EModules.SystemApply, EFunctions.SystemMng, EActions.Del) != 1)
                throw new Exception(String.Format("Service Method:{0} Access Error", base.GetActionName()));
